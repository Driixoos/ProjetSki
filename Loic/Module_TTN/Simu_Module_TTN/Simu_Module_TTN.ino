...
void setup() {
  ...
  delay(100);
  Serial.println("\n[BOOT] Démarrage du module ESP32...");

  // Lire et afficher ID + remontée
  lireID();
  Serial.printf("[INFO] Remontée calculée : %d\n", remontee);

  // Vérifie si c'est un premier pylône
  if ((module_id - 1) % 10 == 0) {
#ifndef AUTO_PIN_MAP
    ...
    Serial.println("[INFO] Ce module n'est pas un premier pylône, pas d'envoi TTN.");
  }
}
...
void loop() {
  ...
  // Affiche toujours les données sur le moniteur série
  Serial.printf("[DONNÉES] Température = %.1f°C | Vitesse = %.1f km/h | Direction = %.1f°\n",
  ...
  if ((module_id - 1) % 10 == 0) {
    ...
    if (ttn.sendBytes(lpp.getBuffer(), lpp.getSize())) {
      Serial.printf("[SEND] Temp: %.1f°C | Vent: %.1f km/h | Dir: %.1f° | ID: %d | Remontee: %d\n",
        temperature, windSpeed, windDir, module_id, remontee);
    } else {
      Serial.println("[ERROR] Échec d'envoi vers TTN");
    }
  }

  delay(10000); // envoi toutes les 10 secondes si premier pylône
}